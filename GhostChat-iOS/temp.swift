...
class ViewController: UIViewController, CBPeripheralManagerDelegate, CBCentralManagerDelegate,CBPeripheralDelegate, UITextFieldDelegate {
    ...
    var messagesDictionary = [ NSUUID : ChatThread]()
    var usernamesDictionary = [NSUUID : String]()
    ...
    func centralManager(central: CBCentralManager!, didDiscoverPeripheral peripheral: CBPeripheral!, advertisementData: [NSObject : AnyObject]!, RSSI: NSNumber!) {
        ...
        /*if let newMessage:AnyObject = advertisementData[CBAdvertisementDataLocalNameKey] {
        myNameString = newMessage as! String
        if myNameString!.hasPrefix("Ghost"){
        println("aaaa")
        var colonRange = myNameString.rangeOfString(":")
        nameString = myNameString.substringWithRange(Range<String.Index>(start:myNameString.startIndex,end:colonRange!.startIndex))
        nameString = nameString.stringByReplacingOccurrencesOfString("Ghost ",withString: "",options: NSStringCompareOptions.LiteralSearch, range: nil)
        //println(nameString)
        
        messageString = myNameString.substringWithRange(Range<String.Index>(start:colonRange!.startIndex,end: myNameString.endIndex))
        messageString = messageString.stringByReplacingOccurrencesOfString(": ", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)
        ...
        //println(nameString)
        //println(messageString)
        if usernamesDictionary[newUUID] != nil {
        //println("here")
        usernamesDictionary[newUUID] = nameString
        messagesDictionary[newUUID]?.username = nameString
        messagesDictionary[newUUID]?.thread.append((true,messageString))
        //println(messagesDictionary[newUUID]?.thread)
        }else{
        usernamesDictionary[newUUID] = nameString
        messagesDictionary[newUUID]?.username = nameString
        messagesDictionary[newUUID]?.thread.append((true,messageString))
